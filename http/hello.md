...
run async 
use waitgrps Time.Sleep(time.Second)


channels: goroutines talk to each other using the same.
bidirectional by default can be made otherwise
chan T (bi)
chan <- T (send)
<- chan T (receive)
compiler error if read*

enforcement by compiler :  The compiler treats send-only and receive-only channels as distinct types, and that type system prevents misuse